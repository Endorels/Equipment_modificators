...
public:
    ...
    virtual void get_modificator(std::vector<modificator> &mods) = 0;
...
private:
   ...
   void get_modificator(std::vector<modificator> &mods)
   {
       for(auto &it : mods)
       {
...
public:
    weapon() = delete;
    weapon(std::shared_ptr<weapon_struct>weapon_str, std::vector<modificator> &mods) : item(weapon_str)
    {
...
public:
    armor() = delete;
    armor(std::shared_ptr<armor_struct>armor_str, std::vector<modificator> &mods) : item(armor_str)
    {
        get_modificator(mods);
    }
    ~armor(){}
